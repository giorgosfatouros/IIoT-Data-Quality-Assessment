...
import logging
import time
from pyLeanxcale import errors, responses_pb2, requests_pb2
from pyphoenix import common_pb2
from importlib import import_module
import ssl
from pyLeanxcale import versions #This file is autogenerated with the setup.py
...
def get_class(kls):
    ...
    parts = kls.split('$')
    class_ = getattr(import_module("pyLeanxcale.responses_pb2"), parts[1])
    rv = class_()
    return rv
...
class AvaticaClient(object):
    ...
    def connect(self):
        ...
        if not self.secure:
            ...
            try:
                ...
                self.connection = httplib.HTTPSConnection(self.url.hostname, self.url.port,
                                                          context=context)
                self.connection.connect()
                self.cookies = {}
                #self.checkSerialization()
    ...
    def checkSerialization(self):
        ...
        while True:
            ...
            try:
                self.connection.request('OPTIONS', '')
                response = self.connection.getresponse()
                response_body = response.read()
                serialization = str(response_body).split(';')
                if len(serialization) < 2 or "customSerialization=PROTOBUF" not in serialization[1]:
                    raise errors.SerializationError('Avatica server is not using PROTOBUF serialization')
                ...
                if index:
                    ...
                    if (int(minimumVersions[0]) > int(versions.MAJOR_VERSION) or int(minimumVersions[1]) > int(versions.MINOR_VERSION)):
                      raise errors.SerializationError("Client version '{}.{}' is not allowed to connect. Minimum allowed version is '{}.{}'".format(versions.MAJOR_VERSION,versions.MINOR_VERSION,minimumVersions[0],minimumVersions[1]))
                ...
                if retry_count > 0:
                    ...
                    time.sleep(delay)
                    continue
                raise errors.InterfaceError('Max retries for attempting to connect and check serialization wasted', cause=e)
            else:
    ...
    def _post_request(self, body, headers):
        ...
        while True:
            ...
            try:
                ...
                if retry_count > 0:
                    print("Error at HTTP connection level... Trying to reconnect")
                    delay = math.exp(-retry_count)
                    logger.debug("HTTP protocol error, will retry in %s seconds...", delay, exc_info=True)
                    self.close()
                    self.connect()
                    print("Successfully reconnected HTTP connection, Continuing...")
                    time.sleep(delay)
                    retry_count -= 1
                    continue
    ...
    def _apply(self, request_data, expected_response_type=None):
        ...
        if message.name == 'org.apache.calcite.avatica.proto.Responses$ErrorResponse':
            # Decode the error response to provide a full Error Message:

            errorresponse = get_class(wire_message.name)
            errorresponse.ParseFromString(wire_message.wrapped_message)

            for exception in errorresponse.exceptions:
                exceptionmessage = exception.split('\n')[0]
                if 'org.apache.calcite.avatica.NoSuchConnectionException' in exceptionmessage:
                    if self.avatica_con is not None:
                        print("No such connection exception. Trying to reconnect...")
                        self.avatica_con.open()
                        print("Successfully opened a new connection")
                        ...
                        if response.headers.get('set-cookie') is not None:
                            self.cookies['hap-cookie'] = response.headers.get('set-cookie')
                            logger.debug(
                                "Setting cookie: {}:{}".format('hap-cookie', response.headers.get('set-cookie')))
                        # deserialize WireMessage
                        wire_message = common_pb2.WireMessage()
                        wire_message.ParseFromString(response_body)
        ...
        elif message.name != expected_response_type:
