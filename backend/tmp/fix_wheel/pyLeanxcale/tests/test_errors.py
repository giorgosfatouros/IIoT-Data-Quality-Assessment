...
class DataErrorTest(DatabaseTestCase):
    ...
    def test_number_outside_of_range(self):
        self.createTable("pyphoenix_test_tbl1", "id SMALLINT primary key")
        with self.conn.cursor() as cursor:
            with self.assertRaises(self.conn.DataError) as cm:
                cursor.execute("UPSERT INTO pyphoenix_test_tbl1 VALUES (60000)")
            self.assertEqual("Type mismatch. SMALLINT and INTEGER for 60000", cm.exception.message)
